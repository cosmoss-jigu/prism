...
RESULT_DIR="${CURRENT_DIR}/result/figure8"		    #Results go this directory.
WORKLOAD_SRC="${ROOT_DIR}/ycsb_generator/figure8"	    #Generated YCSB workloads are in this directory.
...
SVC_SIZE=(64)	    #Size of Scan-aware Value Cache
PWB_SIZE=(64)	    #Size of Persistent Write Buffer
		...
		do
			...
			do
				...
				do
				    ...
				    sed -i "25s/.*/#define MTS_OPLOG_G_SIZE (${pwb_size}UL * 1UL * 1024UL * 1024UL)/g" ${PRISM_DIR}/include/mts-config.h
				    ...
				    sed -i "65s/.*/#define MTS_DRAMCACHE_SIZE ((${svc_size}UL * 1UL * 1024UL * 1024UL))/g" ${PRISM_DIR}/include/mts-config.h
